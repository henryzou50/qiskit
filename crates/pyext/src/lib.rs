...
use qiskit_accelerate::{
    ...
    sparse_pauli_op::sparse_pauli_op, star_block::star_block,
    star_prerouting::star_prerouting, stochastic_swap::stochastic_swap,
...
fn _accelerate(m: &Bound<PyModule>) -> PyResult<()> {
    ...
    m.add_wrapped(wrap_pymodule!(star_block))?;
